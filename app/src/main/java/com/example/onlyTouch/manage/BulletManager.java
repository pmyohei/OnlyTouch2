...
public class BulletManager {
    ...
    public void bulletManage(GL10 gl, ParticleSystem particleSystem, ParticleData particleData) {
        ...
        // 位置が急上昇した境界パーティクルを取得
        int tooRiseIndex = particleData.tooRiseBorderParticle(particleSystem);
        // 保持している境界パーティクルの位置情報を更新
        particleData.updateBorderParticlePosY(particleSystem);
        ...
        for (Bullet bullet : mShotBullets) {
            ...
            //-----------
            if (tooRiseIndex != particleData.NOT_FOUND) {
                float borderY = particleSystem.getParticlePositionY(tooRiseIndex);
